    ...
    */
    describe('cypress/e2e/generated/CASE-1/regression-11', () => {
...
action:dropdown:select - {"options":["driver","doctor","teacher"],"selectedOption":"doctor"}
check:screenshot - 
action:form:submit - 
check:screenshot - 
 ...
 */
        ...
        it('Test body', function() {
          const specFile = __filename;
			...
			cy.log('**Step [3/8]:** action:number-input:type - {"value":"24"}');
		

						cy.get('input[data-test-id="age"]').type(`24`).blur();
					
			cy.log('**Step [4/8]:** action:dropdown:select - {"options":["driver","doctor","teacher"],"selectedOption":"doctor"}');
		

						cy.get('select[data-test-id="profession"]').select('doctor');
